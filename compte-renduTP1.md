...
## Étape 6 — Filtrage et projections avec API Platform

*Module R4.01 – Architecture logicielle. Thème : mise à jour d'une collection (opérations Update et Delete), puis filtrage et projections avec API Platform (branche GitLab `etape-06`).*

## Mise à jour d'une collection : opérations Update et Delete

### Hypothèse et test de suppression

On retire la propriété `orphanRemoval` si elle est présente sur la classe `Auteurs`, puis on exécute la requête suivante en PUT :
...
Le résultat obtenu est correct : la requête aboutit.

On tente ensuite une suppression avec le contenu suivant :
...
Contrairement à ce qui était attendu, cette requête fonctionne dans cet essai.

Il est donc nécessaire de modifier la liste de suppression (`removeList`).

### Expérience : suppression en cascade avec orphanRemoval

On ajoute la propriété `orphanRemoval=true` sur l'entité `Auteur` afin d'éviter les livres orphelins :
...
## Filtrage et recherche avec API Platform

### Contexte

À ce stade, nous savons :

- comment exposer une entité en tant que ressource API ;
- exploiter plusieurs autres fonctionnalités déjà abordées d'API Platform.

Mais qu'est-ce que le filtrage ?

Notre client API — qui pourrait par exemple être une application React — ne souhaite pas toujours récupérer l'ensemble des auteurs ou des livres du système.

Que se passe-t-il si l'on a besoin de ne récupérer que les livres dont le titre contient un mot donné, ou filtrés par année de publication, ou encore les auteurs dont le nom contient une chaîne de caractères donnée ?

C'est précisément ce que permet le filtrage, à l'aide de filtres.

## Filtrage par recherche partielle sur le titre de l'entité `Livre`
...
Le mode `partial` permet de rechercher le terme n'importe où dans le titre.

### Résultat obtenu

Le filtre fonctionne comme attendu. Le tri par défaut des résultats se fait par identifiant croissant.

## Filtrage par intervalle de valeurs sur l'année de l'entité `Livre`
...
Pour retrouver l'instruction d'import complète, il suffit de placer le curseur sur le nom de la classe dans l'éditeur : l'IDE propose alors automatiquement l'import correspondant.

```php 
use ApiPlatform\Core\Bridge\Doctrine\Orm\Filter\RangeFilter;
```

L'IDE affiche une icône de suggestion (trois petits points) en bas à droite du nom de la classe ; il suffit de cliquer dessus, puis d'utiliser Ctrl+F pour rechercher l'élément souhaité.

## Filtrage sur les propriétés : réaliser des projections (sélection des attributs retournés)

L'objectif est de réduire la taille affichée d'une biographie en proposant un court résumé, accompagné d'un bouton « voir plus » permettant d'afficher la biographie complète.
...
- Les propriétés `shortBiographie` et `biographie` sont exposées simultanément, ce qui n'est pas satisfaisant.
- Il faudrait que seule `shortBiographie` soit visible dans la réponse.
...
Si l'on demande un attribut qui n'existe pas, celui-ci est simplement ignoré : la requête aboutit tout de même.

### Renommer une propriété exposée avec SerializedName

Pour renommer la propriété exposée `shortBiographie` en `biographie` dans la réponse JSON, on utilise l'annotation `@SerializedName` :
...
## Filtrage et recherche dans les relations

### Contexte

Actuellement, si l'on exécute `/api/auteurs/5.jsonld`, on obtient :

### Configuration (1)

On utilise un tableau de tableaux pour la propriété `properties`.
...
Cette requête fonctionne comme attendu.

On ajoute par ailleurs le groupe `auteurs:item:get` aux propriétés concernées :
...
### Configuration (2) : ajout du filtrage par auteur dans l'entité Livre
